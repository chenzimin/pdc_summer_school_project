...
int main(int argc, char *argv[]) {
    ...
    for( i=0; i<num_storms; i++) {
        ...
        /* For each particle */
	#pragma omp  parallel for default(shared) private(j, k, energy, position) reduction(+:layer[:layer_size])
        for( j=0; j<storms[i].size; j++ ) {
            ...
            for( k=0; k<layer_size; k++ ) {
                ...
                update( layer, layer_size, k, storms[i].posval[j*2], (float)storms[i].posval[j*2+1] * 1000);
            }
        }
