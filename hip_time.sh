...
for i in {1..6}; do
    ...
    output_file="${directory}/hip_a30k_w${i}_time.txt"
    for times in {1..10}; do
        srun -n 1 "${curent_dir}/energy_storms_hip" 30000 ${test_files} >> "${output_file}"
    done
done
...
mkdir -p "${directory}"
for i in {1..15}; do
    output_file="${directory}/hip_a"${num_layer}"_w1_time.txt"
    for times in {1..10}; do
        srun -n 1 "${curent_dir}/energy_storms_hip" ${num_layer} ${test_files} >> "${output_file}"
    done
