...
for i in {1..6}; do
    ...
    output_file="${directory}/mpi_a30k_w${i}_time.txt"
    for times in {1..10}; do
        srun -n 32 "${curent_dir}/energy_storms_mpi" 30000 ${test_files} >> "${output_file}"
    done
done
...
mkdir -p "${directory}"
for i in {1..15}; do
    output_file="${directory}/mpi_a"${num_layer}"_w1_time.txt"
    for times in {1..10}; do
        srun -n 32 "${curent_dir}/energy_storms_mpi" ${num_layer} ${test_files} >> "${output_file}"
    done
